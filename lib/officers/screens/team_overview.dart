...
class WorkersCache {
  ...
  List<Map<String, dynamic>> hseWorkers = []; // Keep internal name for consistency
...
class _WorkersListScreenState extends State<WorkersListScreen>
  ...
  Future<void> _deleteWorker(String tableName, String workerId) async {
    ...
    if (confirm == true) {
      try {
        ...
        _showSnackBar('Deleted offline - will sync when online.', Colors.orange);
      } catch (e) {
        if (!mounted) return; // Check mounted in catch block
              ...
              setModalState(() {
                if (query.isEmpty) {
                  ...
                  filteredSites = cache.availableSites
                      .where((site) => (site['name'] as String)
                      .toLowerCase()
              ...
              child: BackdropFilter(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border(
                      // ✅ Replaced withOpacity
                      top: BorderSide(color: Colors.white.withAlpha((255 * 0.2).round())),
                    ),
                  ),
                  child: Padding(
  ...
  Future<void> _updateWorkerSite(
      String tableName, String workerId, dynamic newSiteId) async {
    if (!mounted) return; // Check mounted

    try {
      await Supabase.instance.client
          .from(tableName)
          .update({'current_site_id': newSiteId}).eq('id', workerId);
      ...
      _showSnackBar('Site change saved offline - will sync when online.', Colors.orange);
    } catch (e) {
  ...
  Widget _buildWorkerCard(
    ...
    final name = "${capitalizeName(firstName)} ${capitalizeName(lastName)}".trim();
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: _buildWorkerInfo(name, subtitle, siteName),
                ),
