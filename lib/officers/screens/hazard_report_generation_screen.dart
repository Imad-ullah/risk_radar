...
class _ResolvedHazardDetailsScreenState
  ...
  Future<void> _fetchAssignedWorkerName() async {
    final assignedUid = widget.hazard['assigned_to'] ?? widget.hazard['worker_id'];
    if (assignedUid == null) {
  ...
  String _getReporterName() {
    if (widget.hazard['reporter_name'] != null && widget.hazard['reporter_name'].toString().isNotEmpty) {
      return widget.hazard['reporter_name'];
    }
    if (widget.hazard['reporter'] != null) {
      return _formatName(widget.hazard['reporter']['first_name'], widget.hazard['reporter']['last_name'], "Unknown");
    }
  ...
  Future<Uint8List> _generatePdfBytes() async {
    ...
    // Load Before Images
    try {
      final String? rawBeforeUrl = widget.hazard['image_url'] ?? widget.hazard['images']?.toString();
      if (rawBeforeUrl != null && rawBeforeUrl.isNotEmpty) {
        final urls = rawBeforeUrl.split(',');
        for (String url in urls) {
          if (url.trim().isNotEmpty) beforeImages.add(await networkImage(url.trim()));
        }
      }
    ...
    try {
      ...
      if (rawAfterUrl != null && rawAfterUrl.isNotEmpty) {
        ...
        for (String url in urls) {
          if (url.trim().isNotEmpty) afterImages.add(await networkImage(url.trim()));
        }
      }
        ...
        build: (pw.Context context) {
          return [
            pw.Row(
              ...
              children: [
                pw.Expanded(child: _buildPdfPersonnelBox("REPORTED BY", _getReporterName(), widget.hazard['reporter']?['work_type'] ?? "Worker")),
                pw.SizedBox(width: 20),
                pw.Expanded(child: _buildPdfPersonnelBox("RESOLVED BY", _formatName(_resolverData?['first_name'], _resolverData?['last_name'], "Unassigned"), _resolverData?['designation'] ?? "Specialist")),
              ],
            ),
            ...
            pw.Row(
              ...
              children: [
                ...
                _buildPdfMetricBox("SEVERITY", widget.hazard['severity'] ?? "LOW"),
                pw.SizedBox(width: 10),
                _buildPdfMetricBox("IMPACT SCORE", "${(_getSeverityValue(widget.hazard['severity']) * 100).toInt()}%"),
              ],
            ),
            ...
            pw.TableHelper.fromTextArray(
              ...
              data: [
                ...
                ['Location', '${widget.hazard['latitude'] ?? 'N/A'}, ${widget.hazard['longitude'] ?? 'N/A'}'],
                ['Reported At', reportedDate],
                ['Started At', startedDate],
                ['Resolved At', resolvedDate],
                ['Resolution Notes', widget.hazard['resolution_notes'] ?? "N/A"],
              ],
    ...
    // Image Pages
    if (beforeImages.isNotEmpty) _addPdfImagePage(pdf, "VISUAL VERIFICATION: INITIAL HAZARD (BEFORE)", beforeImages, displayId, logoImage);
    if (afterImages.isNotEmpty) _addPdfImagePage(pdf, "VISUAL VERIFICATION: RESOLUTION PROOF (AFTER)", afterImages, displayId, logoImage);

    return pdf.save();
  }

  void _addPdfImagePage(pw.Document pdf, String title, List<pw.ImageProvider> images, String displayId, pw.ImageProvider? logoImage) {
    pdf.addPage(
        pw.MultiPage(
            pageFormat: PdfPageFormat.a4,
            margin: const pw.EdgeInsets.all(32),
            ...
            build: (pw.Context context) {
              return [
                pw.Text(title, style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold, color: PdfColor.fromHex('#1B3D3D'))),
                pw.SizedBox(height: 10),
                pw.Wrap(
                  spacing: 15,
                  runSpacing: 15,
      ...
      children: [
        pw.Row(
          ...
          children: [
            ...
            if (logoImage != null)
              pw.Container(height: 80, width: 140, alignment: pw.Alignment.centerRight, child: pw.Image(logoImage, fit: pw.BoxFit.contain)),
          ],
        ),
        pw.SizedBox(height: 10),
  ...
  Future<void> _downloadReport() async {
    ...
    try {
      ...
      await Printing.layoutPdf(
        ...
        name: 'RiskRadar_Audit_${widget.hazard['report_number'] ?? widget.hazard['id'].toString().substring(0, 6)}.pdf',
      );
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _shareReport() async {
    ...
    try {
      ...
      final fileName = 'RiskRadar_Audit_${widget.hazard['report_number'] ?? widget.hazard['id'].toString().substring(0, 6)}.pdf';

      await SharePlus.instance.share(
        ShareParams(
          files: [XFile.fromData(bytes, name: fileName, mimeType: 'application/pdf')],
          subject: 'Safety Report: ${widget.hazard['hazard_type']}',
          text: 'Please find the attached Safety Compliance Audit for RiskRadar.',
        ),
      );
    } catch (e) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final String description = hazard['description'] ?? 'No description provided.';
    final String severity = hazard['severity'] ?? 'LOW';
    final String status = hazard['status'] ?? 'Unknown';

    final double? latitude = double.tryParse(hazard['latitude']?.toString() ?? '');
    final double? longitude = double.tryParse(hazard['longitude']?.toString() ?? '');
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 1,
        actions: [
          IconButton(icon: const Icon(Icons.download_outlined, color: Colors.white), onPressed: _downloadReport),
          IconButton(icon: const Icon(Icons.share_outlined, color: Colors.white), onPressed: _shareReport),
        ],
      ),
          ...
          children: [
            ...
            Row(
              children: [
                ...
                _buildPersonnelBadge(
                  label: "RESOLVED BY",
                  name: _formatName(_resolverData?['first_name'], _resolverData?['last_name'], "Unassigned"),
            ...
            // --- Summary Section ---
            Text("RESOLUTION SUMMARY", style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: isDark ? Colors.white : AppColors.brandTeal, letterSpacing: -0.5)),
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 12),
                _buildMetricCard("RISK SEVERITY", severity.toUpperCase(), Icons.assessment_outlined),
              ],
            ),
            const SizedBox(height: 25),
            ...
            _buildClickableGallery("Initial Hazard (Before)", hazard['image_url'] ?? hazard['images']?.toString(), isDark),
            const SizedBox(height: 15),
            _buildClickableGallery("Resolution Proof (After)", hazard['resolution_image_url'], isDark),

            // --- Voice Notes ---
            if (hazard['voice_note_url'] != null || hazard['resolution_voice_note_url'] != null) ...[
              const SizedBox(height: 30),
              _buildSectionLabel("AUDIO TESTIMONY", isDark),
              if (hazard['voice_note_url'] != null)
                _buildSimpleCard("Original Voice Report", _buildVoiceList(hazard['voice_note_url']), isDark),
              const SizedBox(height: 12),
              if (hazard['resolution_voice_note_url'] != null)
                _buildSimpleCard("Resolution Testimony", _buildVoiceList(hazard['resolution_voice_note_url']), isDark),
            ],
  ...
  Widget _buildPersonnelBadge({required String label, required String name, required String? imageUrl, required String role, required IconData fallbackIcon, required bool isDark}) {
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            CircleAvatar(
              radius: 28,
              backgroundColor: isDark ? Colors.grey.shade800 : AppColors.backgroundLight,
        ...
        children: [
          SizedBox(
            ...
            child: PieChart(PieChartData(sectionsSpace: 0, centerSpaceRadius: 25, sections: [
              PieChartSectionData(value: risk, color: _getSeverityColor(severity), radius: 12, showTitle: false),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text("${(risk * 100).toInt()}%", style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: _getSeverityColor(severity))),
  ...
  Widget _buildAuditTable(Map<String, dynamic> hazard, String description, bool isDark) {
    return Container(
      decoration: BoxDecoration(color: Theme.of(context).cardColor, borderRadius: BorderRadius.circular(15), border: Border.all(color: isDark ? Colors.grey.shade800 : Colors.transparent)),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(15),
        child: Table(
          ...
          children: [
            _buildTableRow("DATA FIELD", "ENTRY LOG", isHeader: true, isDark: isDark),
            _buildTableRow("Description", description, isDark: isDark),
            _buildTableRow("Location", "${hazard['latitude'] ?? 'N/A'}, ${hazard['longitude'] ?? 'N/A'}", isDark: isDark),
            _buildTableRow("Reported At", _formatDate(hazard['created_at']), isDark: isDark),
            _buildTableRow("Assigned At", _formatDate(hazard['assigned_at']), isDark: isDark),
            _buildTableRow("Resolved At", _formatDate(hazard['resolved_at']), isDark: isDark),
            _buildTableRow("Resolution Notes", hazard['resolution_notes'] ?? "N/A", isDark: isDark),
          ],
        ),
      ),
    );
  ...
  Widget _buildBadge(String t, Color c) => Container(
      ...
      decoration: BoxDecoration(color: c.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(20), border: Border.all(color: c.withValues(alpha: 0.5))),
  ...
  Widget _buildSectionLabel(String t, bool isDark) => Padding(
      ...
      child: Text(t, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: isDark ? Colors.white54 : AppColors.brandTeal.withValues(alpha: 0.4), letterSpacing: 1.5))
  );

  Widget _buildSimpleCard(String label, List<Widget> children, bool isDark) => Container(
      width: double.infinity, padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: Theme.of(context).cardColor, borderRadius: BorderRadius.circular(15), border: Border.all(color: isDark ? Colors.grey.shade800 : Colors.transparent)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey)), ...children])
  );
  ...
  TableRow _buildTableRow(String l, String v, {bool isHeader = false, required bool isDark}) {
    return TableRow(
  ...
  String _calculateTime() {
    if (widget.hazard['created_at'] == null || widget.hazard['resolved_at'] == null) return "N/A";
    try {
      String startRaw = widget.hazard['created_at'].toString().split('+')[0].split('Z')[0].replaceFirst(' ', 'T');
      String endRaw = widget.hazard['resolved_at'].toString().split('+')[0].split('Z')[0].replaceFirst(' ', 'T');
  ...
  Color _getStatusColor(String? s) {
    switch (s?.toLowerCase()) {
      case 'resolved': return _successGreen;
      case 'in_progress':
