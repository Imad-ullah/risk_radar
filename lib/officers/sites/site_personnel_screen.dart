...
class _SitePersonnelScreenState extends State<SitePersonnelScreen>
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        _filteredWorkers = _workers.where((worker) {
          ...
          final searchableString = "$firstName $lastName $workType".toLowerCase();
  ...
  Future<void> _loadPersonnelCacheFirst() async {
    final cached = OfficerRepository.instance.getOfficerSitePersonnel(widget.siteId);
    if (cached != null) {
      _workers = List<Map<String, dynamic>>.from(cached['workers'] ?? []);
      _hseWorkers = List<Map<String, dynamic>>.from(cached['hse_workers'] ?? []);
      _filterLists();
  ...
  Future<void> _loadPersonnelData({bool showBlockingLoader = true}) async {
    ...
    try {
      ...
      final responses = await Future.wait([
        ...
        supabase.from('hse_workers').select().eq('current_site_id', widget.siteId),
      ]);
      ...
      await OfficerRepository.instance.saveOfficerSitePersonnel(
        widget.siteId,
        {
          'workers': _workers,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error loading personnel: $e')),
        );
      }
    }
  }
  ...
  Widget _buildHighlightedText(String text, String query, TextStyle defaultStyle) {
    if (query.isEmpty) {
      return Text(
        text,
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Column(
            ...
            children: [
              Text(widget.siteName, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
              const Text("Site Workforce", style: TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.normal)),
            ],
          ),
          centerTitle: true,
          ...
          bottom: const TabBar(
            ...
            tabs: [
              ...
              Tab(icon: Icon(Icons.health_and_safety_rounded), text: 'Safety Officers'),
            ],
          ),
        ),
        body: _isLoading
          ...
          children: [
            ...
            Expanded(
              child: FadeTransition(
                ...
                child: TabBarView(
                  children: [
                    _buildPersonnelGrid(_filteredWorkers, 'No workers assigned to this site', 'workers'),
                    _buildPersonnelGrid(_filteredHseWorkers, 'No safety officers assigned to this site', 'hse_workers'),
                  ],
                ),
              ),
            ),
  ...
  Widget _buildPersonnelGrid(List<Map<String, dynamic>> personnelList, String emptyMessage, String tableName) {
